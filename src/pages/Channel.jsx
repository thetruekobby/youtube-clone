...
const Channel = () => {
  ...
  return (
    <div>
      {!channelVideos && <ErrorMessage />}

      {/* banner */}
      {channelData?.[0]?.brandingSettings?.image?.bannerExternalUrl && (
        <div className="aspect-[16/3] bg-gray-500/10 mb-6 border-none">
          <img src={channelData?.[0]?.brandingSettings?.image?.bannerExternalUrl} className="w-full h-full object-cover object-center" />
        </div>
      )}
      <Stack direction={{ xs: "row" }} spacing={5} alignItems={"center"} mb={5}>
        {channelData?.[0]?.snippet?.thumbnails?.medium?.url && (
          <Avatar
        ...
        {channelData && (
          <Box>
            ...
            <Typography variant="body1" className="text-[var(--clr-gray-text)]">
              {channelData?.[0]?.snippet?.customUrl} {channelData?.[0]?.statistics?.subscriberCount} subscribers{" "}
              {channelData?.[0]?.statistics?.videoCount} videos
            </Typography>
            <Typography variant="body1" className="text-[var(--clr-gray-text)] mb-10">
        ...
        <Stack direction={{ xs: "row" }} alignItems={"flex-start"} justifyContent="center" flexWrap={"wrap"} spacing={3} useFlexGap>
          {channelVideos &&
            channelVideos.map((video, index) => (
              ...
              >
                <Box className="aspect-video bg-gray-500/10 rounded overflow-hidden mb-2">
                  <img src={video.snippet?.thumbnails?.high?.url} className="w-full h-full" />
                </Box>
                <Typography variant="body2">{video.snippet?.title}</Typography>
                <Stack direction={"row"} spacing={2} alignItems={"center"} className="text-[var(--clr-gray-text)]">
                  <Typography variant="caption">{numFormatter(Math.floor(Math.random() * 1000000) + 1)} views</Typography>
                  <p className="bg-[var(--clr-gray-text)] rounded-full aspect-square w-1 "></p>
